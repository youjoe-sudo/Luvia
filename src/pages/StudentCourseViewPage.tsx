...
export default function StudentCourseViewPage() {
  ...
  const handleWatchVideo = () => {
    if (selectedLesson?.google_drive_video_id) {
      ...
      toast({
        title: "عذراً",
        description: "رابط المحاضرة غير متوفر",
        variant: "destructive",
      });
    }
  };
  ...
  const executeSubmission = async () => {
    ...
    try {
      const formatted = Object.entries(assignmentAnswers).map(
        ([qId, opts]) => ({
          question_id: qId,
      ...
      <div className="container py-6 md:py-10 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          ...
          <Badge className="bg-blue-600/20 text-blue-400 border-blue-500/20 px-4 py-2 rounded-xl text-sm font-bold">
            {t("التقدم:", "Progress:")}{" "}
            {Math.round(
              (completedLessons.size / (course?.lessons?.length || 1)) * 100,
            )}
            %
          </Badge>
        </div>
              ...
              >
                <Card className="bg-[#0f172a] border-white/5 rounded-3xl overflow-hidden shadow-2xl mb-6 relative w-full group">
                  <div className="relative w-full aspect-video flex flex-col items-center justify-center bg-gradient-to-t from-black to-blue-950/30">
                    ...
                    <h2 className="mt-6 text-xl font-black text-white px-4 text-center z-10">
                      {language === "ar"
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setCompletedLessons((prev) =>
                        new Set(prev).add(selectedLesson.id),
                      );
                ...
                >
                  {markingComplete ? (
                    ...
                    <>
                      ...
                      {t("تم إكمال المحاضرة", "Completed")}
                    </>
                  ) : (
                    <>
                      <CheckCircle className="w-6 h-6" />{" "}
                      {t("أنهيت المشاهدة؟ افتح الواجب", "Unlock Assignment")}
                    </>
                  )}
                </Button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      assignment.questions.length > 0 ? (
                        assignmentSubmission ? (
                          ...
                          <Button
                            ...
                            className="bg-purple-600 hover:bg-purple-500 rounded-lg h-8 px-4"
                          >
                            {t("ابدأ", "Start")}
                          </Button>
                        )
                  ...
                  <Card
                    className={`bg-white/[0.03] border-white/5 rounded-2xl p-6 flex items-center justify-between ${!currentPdfUrl ? "opacity-70" : "cursor-pointer hover:bg-white/[0.05]"}`}
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-bold text-white text-sm">
                          {t("الملحقات", "Resources")}
                        </h4>
                        <p className="text-[10px] text-slate-500">
                          {currentPdfUrl
                            ? t("تحميل ملخص PDF", "Download PDF")
                            : t("لم يتم رفع ملفات", "No files uploaded")}
                        </p>
                      </div>
                    </div>
                    {currentPdfUrl ? (
        ...
        <DialogContent className="max-w-4xl w-full h-[100dvh] md:h-[85vh] bg-[#020617] border-none md:border md:border-white/10 p-0 flex flex-col overflow-hidden md:rounded-[3rem]">
          <div className="p-5 md:p-8 border-b border-white/5 bg-white/5 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <DialogTitle className="text-base md:text-xl font-black text-white">
                  {language === "ar"
                    ? assignment?.title_ar
                    : assignment?.title_en}
                </DialogTitle>
                <div className="w-24 h-1 bg-white/5 rounded-full mt-2 overflow-hidden">
                  <motion.div
                    ...
                    animate={{
                      width: `${((currentQuestionIndex + 1) / (assignment?.questions?.length || 1)) * 100}%`,
                    }}
                  />
                </div>
              </div>
            ...
            <DialogClose asChild>
              <Button
                variant="ghost"
                className="w-10 h-10 rounded-xl bg-white/5 text-slate-400"
              >
                <X className="w-5 h-5" />
              </Button>
            </DialogClose>
              ...
              {assignment?.questions?.[currentQuestionIndex] && (
                ...
                >
                  ...
                  <div className="space-y-3">
                    {assignment.questions[currentQuestionIndex].options.map(
                      (opt) => {
                        const isSelected = assignmentAnswers[
                          assignment.questions[currentQuestionIndex].id
                        ]?.includes(opt.id);
                        return (
                          <Label
                            key={opt.id}
                            className={`flex items-center gap-4 p-5 rounded-2xl border-2 transition-all cursor-pointer ${isSelected ? "bg-purple-600/20 border-purple-500 text-white" : "bg-white/5 border-white/5 text-slate-400 hover:bg-white/10"}`}
                          >
                            <Checkbox
                            ...
                            />
                            <div
                              className={`shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center ${isSelected ? "bg-purple-500 border-purple-500" : "border-slate-700"}`}
                            >
                              {isSelected && (
                            ...
                            <span className="text-base font-medium">
                              {language === "ar" ? opt.text_ar : opt.text_en}
                            </span>
                          </Label>
                        );
                      },
          ...
          <div className="p-5 md:p-8 border-t border-white/5 bg-white/5 flex items-center justify-between">
            ...
            (assignment?.questions?.length || 0) - 1 ? (
              <Button
                ...
                className="bg-green-600 hover:bg-green-500 text-white px-8 rounded-xl font-bold"
              >
                تسليم
              </Button>
            ) : (
      ...
      <Dialog open={showResult} onOpenChange={setShowResult}>
        <DialogContent className="max-w-md bg-[#020617] border-white/10 rounded-[2.5rem] p-10 text-center">
          ...
          <Button
            ...
            className="w-full h-14 bg-white text-black rounded-2xl font-black"
          >
            إغلاق
          </Button>
        </DialogContent>
